...
description: React/Next.js performance optimization patterns ordered by impact
license: MIT
...
ordered by impact. Sections 1-2 yield the highest gains (2-10×
...
2. [Bundle Size Optimization](#2-bundle-size-optimization) — **CRITICAL**
   - 2.1 [Dynamic Imports for Heavy Components](#21-dynamic-imports-for-heavy-components)
   - 2.2 [Preload Based on User Intent](#22-preload-based-on-user-intent)
...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   ...
   - 3.3 [Per-Request Deduplication with React.cache()](#33-per-request-deduplication-with-reactcache)
...
5. [Re-render Optimization](#5-re-render-optimization) — **MEDIUM**
   - 5.1 [Subscribe to Derived State](#51-subscribe-to-derived-state)
   - 5.2 [Narrow Effect Dependencies](#52-narrow-effect-dependencies)
   - 5.3 [Use Transitions for Non-Urgent Updates](#53-use-transitions-for-non-urgent-updates)
   - 5.4 [Extract to Memoized Components](#54-extract-to-memoized-components)
   - 5.5 [Defer State Reads to Usage Point](#55-defer-state-reads-to-usage-point)
6. [Rendering Performance](#6-rendering-performance) — **MEDIUM**
   - 6.1 [Hoist Static JSX Elements](#61-hoist-static-jsx-elements)
   - 6.2 [Use Activity Component for Show/Hide](#62-use-activity-component-for-showhide)
   - 6.3 [CSS content-visibility for Long Lists](#63-css-content-visibility-for-long-lists)
   - 6.4 [Optimize SVG Precision](#64-optimize-svg-precision)
7. [JavaScript Performance](#7-javascript-performance) — **LOW-MEDIUM**
   - 7.1 [Combine Multiple Array Iterations](#71-combine-multiple-array-iterations)
   - 7.2 [Cache Property Access in Loops](#72-cache-property-access-in-loops)
   - 7.3 [Use Set/Map for O(1) Lookups](#73-use-setmap-for-o1-lookups)
   - 7.4 [Early Exit from Loops](#74-early-exit-from-loops)
   - 7.5 [Hoist RegExp Creation](#75-hoist-regexp-creation)
   - 7.6 [Cache Storage API Calls](#76-cache-storage-api-calls)
   - 7.7 [Build Index Maps for Repeated Lookups](#77-build-index-maps-for-repeated-lookups)
...
off-screen items (10× faster initial render).
...
Use a Map (not a hook) so it works everywhere: utilities,
event handlers, not just React components.
...
**Important:** If storage can change externally (another tab,
server-set cookies), invalidate cache:
