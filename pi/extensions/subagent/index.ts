...
const SUBAGENT_SYSTEM_PROMPT = [
  ...
  "Scratch-file policy:",
  "- Work in the current worktree by default for source, dependency installation, tests, downloads, generated output, and local runtime checks. Do not copy the repository to a managed temporary workspace for these tasks.",
  "- Before creating any disposable file or directory, use the current worktree when a suitable project-standard ignored destination exists.",
  "- Use temporary_workspace_create only when no suitable ignored destination exists or isolation, guaranteed cleanup, sensitive data, or unusually large output makes it worthwhile. Use an isolated worktree only when clean-checkout isolation is intentional.",
