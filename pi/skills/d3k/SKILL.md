---
name: "d3k"
description: "Use for running, testing, or debugging a local web application when its dev server can run in the current workspace, especially when reproducing browser issues or correlating UI behavior with server logs, console errors, network activity, interactions, and screenshots. Own the local runtime: reuse or background-start d3k, drive its managed Chrome browser, and inspect unified evidence. Do not use for external/deployed-only sites, cross-browser testing, Playwright test authoring, request mocking, or tracing."
compatibility: "Requires d3k (dev3000) and Node.js 24 or newer."
metadata:
  source: "https://github.com/vercel-labs/dev3000/blob/v0.0.178/src/skills/d3k/internal-skill.md"
  version: "0.0.178"
---
...
2. If d3k is not installed, install the pinned runtime:

```bash
bun install -g dev3000@0.0.178 --registry https://registry.npmjs.org
```

Use `npm install -g dev3000@0.0.178 --registry https://registry.npmjs.org` only when Bun is unavailable.
